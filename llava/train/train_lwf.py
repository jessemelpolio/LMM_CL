...
import tokenizers
import deepspeed
# <<< Added DeepSpeed Import >>>
from deepspeed.runtime.zero.partition_parameters import ZeroParamStatus
from deepspeed import zero
# <<< End Added DeepSpeed Import >>>

from transformers import AutoConfig
try:
    # Optional: ensure correct config class when targeting Qwen2
...
from llava.train.llava_lwf_trainer import LLaVALwfTrainer # Use the correctly named LLaVALwfTrainer

from llava import conversation as conversation_lib
from llava.model import *
from llava.mm_utils import process_highres_image, process_anyres_image, process_highres_image_crop_split, tokenizer_image_token
from llava.utils import rank0_print, process_video_with_pyav, process_video_with_decord
from llava.model.language_model.llava_qwen_lwf import LlavaQwenLwfForCausalLM # Import LwF model
...
class ModelArguments:
    ...
    mm_language_model_train_attn_proj_only: Optional[bool] = field(
        default=False, metadata={"help": "When mm_language_model is in mm_tunable_parts, specify whether to only train the self-attention projection layers of the language model. Default is False."}
    )
    mm_language_model_train_mlp_gate_up_only: Optional[bool] = field(
        default=False, metadata={"help": "When mm_language_model is in mm_tunable_parts, train only MLP gate and up projections (keep down projection frozen). Default is False."}
    )
    mm_vision_tower_train_sa_proj_only: Optional[bool] = field(
        default=False, metadata={"help": "When mm_vision_tower is in mm_tunable_parts, train only the self-attention projection layers (q_proj, k_proj, v_proj, out_proj) of the vision tower. Default is False."}
    )
    ...
    teacher_model_path: Optional[str] = field(default=None, metadata={"help": "Path to the teacher model for LwF."})
    lwf_lambda: float = field(default=0.0, metadata={"help": "Lambda for LwF distillation loss."})
    lwf_temperature: float = field(default=1.0, metadata={"help": "Temperature for LwF distillation."})
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if any(
        ...
        # If loading a Qwen target, ensure we use Qwen2Config to satisfy newer fields (e.g., layer_types)
        try:
    ...
    # Add LwF parameters to customized_kwargs for model loading
    if model_args.teacher_model_path is not None and model_args.lwf_lambda > 0:
        rank0_print(f"LLaVA-LwF: teacher_model_path is set to {model_args.teacher_model_path}. lwf_lambda = {model_args.lwf_lambda}. lwf_temperature = {model_args.lwf_temperature}")
        customized_kwargs["config"].teacher_model_path = model_args.teacher_model_path
        customized_kwargs["config"].lwf_lambda = model_args.lwf_lambda
        customized_kwargs["config"].lwf_temperature = model_args.lwf_temperature
        # Set model_type for LwF model explicitly if not already set by a class_name
        if model_args.model_class_name is None and "qwen" in model_args.model_name_or_path.lower():
            customized_kwargs["config"].model_type = "llava_qwen_lwf"
    else:
        # Ensure these are not passed if LwF is not active
        if "config" in customized_kwargs and customized_kwargs["config"] is not None:
            if hasattr(customized_kwargs["config"], "teacher_model_path"):
                delattr(customized_kwargs["config"], "teacher_model_path")
            if hasattr(customized_kwargs["config"], "lwf_lambda"):
                delattr(customized_kwargs["config"], "lwf_lambda")
            if hasattr(customized_kwargs["config"], "lwf_temperature"):
                delattr(customized_kwargs["config"], "lwf_temperature")
    ...
    if model_args.model_class_name is not None:
        ...
        # For LwF, we should be using the LlavaQwenLwfForCausalLM class
        # Check if LwF parameters are present in model_args, indicating an LwF run.
        is_lwf_run = model_args.teacher_model_path is not None and model_args.lwf_lambda > 0

        if "qwen" in model_args.model_name_or_path.lower() and is_lwf_run:
            rank0_print(f"LLaVA-LwF: Initializing LlavaQwenLwfForCausalLM with LwF specific config.")
            model = LlavaQwenLwfForCausalLM.from_pretrained(
                model_args.model_name_or_path,
                cache_dir=training_args.cache_dir,
                attn_implementation=training_args.attn_implementation,
                torch_dtype=(torch.bfloat16 if training_args.bf16 else None),
                low_cpu_mem_usage=False,
                **customized_kwargs, # This now includes LwF config
            )
    ...
    # --- Added: Explicit Teacher Model Loading ---
    if model_args.teacher_model_path is not None and model_args.lwf_lambda > 0:
        rank0_print(f"LLaVA-LwF: Explicitly loading teacher model from {model_args.teacher_model_path}...")
        # Load teacher using the non-LwF class (LlavaQwenForCausalLM)
        # Filter customized_kwargs to avoid passing LwF specific params meant for the student
        teacher_customized_kwargs = {
            k: v for k, v in customized_kwargs.items()
            if k not in ['teacher_model_path', 'lwf_lambda', 'lwf_temperature']
        }
        # If student config has LwF specific type, create/load a clean config for teacher
        teacher_config_for_load = AutoConfig.from_pretrained(model_args.teacher_model_path, cache_dir=training_args.cache_dir)
        # Remove potential LwF attributes from teacher config to be safe
        for attr in ['teacher_model_path', 'lwf_lambda', 'lwf_temperature', 'model_type']:
             if hasattr(teacher_config_for_load, attr):
                  delattr(teacher_config_for_load, attr)

        # Explicitly import the non-LwF model class
        from llava.model.language_model.llava_qwen import LlavaQwenForCausalLM
        
        rank0_print("teacher_customized_kwargs: ", teacher_customized_kwargs)

        loaded_teacher_model = LlavaQwenForCausalLM.from_pretrained(
             model_args.teacher_model_path,
            #  config=teacher_config_for_load, # Use cleaned config
             cache_dir=training_args.cache_dir,
             attn_implementation=training_args.attn_implementation, # Use same attn as student
             torch_dtype=(torch.bfloat16 if training_args.bf16 else None), # Use same dtype as student
             low_cpu_mem_usage=False, # Assuming similar strategy
             **teacher_customized_kwargs # Use filtered kwargs if needed, config might be enough
        )
        # Assign the loaded teacher model to the student model instance
        # Ensure the student model actually has the `teacher_model` attribute initialized (should be done in its __init__)
        if hasattr(model, 'teacher_model'):
             model.teacher_model = loaded_teacher_model
             rank0_print("LLaVA-LwF: Teacher model loaded and assigned to student model.")
        else:
             rank0_print("Warning: Student model does not have 'teacher_model' attribute. Cannot assign loaded teacher.")
...
def train(attn_implementation=None):
    ...
    if hasattr(model, 'teacher_model') and model.teacher_model is not None:
        # Ensure teacher is in eval mode and fully frozen
        model.teacher_model.eval()
        model.teacher_model.requires_grad_(False)
        model.teacher_model.config.use_cache = False
    if model_args.rope_scaling_factor is not None and model_args.rope_scaling_type is not None:
        ...
        if hasattr(model, 'teacher_model') and model.teacher_model is not None:
            model.teacher_model.config.rope_scaling = {
                "factor": model_args.rope_scaling_factor,
                "type": model_args.rope_scaling_type,
            }
    ...
    if training_args.bits in [4, 8]:
        ...
        if hasattr(model, 'teacher_model') and model.teacher_model is not None:
            model.teacher_model.config.torch_dtype = torch.float32 if training_args.fp16 else (torch.bfloat16 if training_args.bf16 else torch.float32)
        model = prepare_model_for_kbit_training(model, use_gradient_checkpointing=training_args.gradient_checkpointing)
        if hasattr(model, 'teacher_model') and model.teacher_model is not None:
            model.teacher_model = prepare_model_for_kbit_training(model.teacher_model, use_gradient_checkpointing=training_args.gradient_checkpointing)
    ...
    if model_args.vision_tower is not None:
        ...
        # --- Added: Initialize Teacher Vision Modules ---
        # Check if the student model instance has the teacher_model attribute AND it's not None
        if hasattr(model, 'teacher_model') and model.teacher_model is not None:
            rank0_print("Initializing teacher model vision modules...")
            # Call initialize_vision_modules on the teacher model instance
            model.teacher_model.get_model().initialize_vision_modules(model_args=model_args, fsdp=training_args.fsdp)
            teacher_vision_tower = model.teacher_model.get_vision_tower()
            # Check if the teacher actually has a vision tower after initialization
            if teacher_vision_tower is not None:
                 teacher_vision_tower.to(dtype=torch.bfloat16 if training_args.bf16 else torch.float16, device=training_args.device)
                 rank0_print("Teacher model vision modules initialized and moved to device.")
            else:
                 rank0_print("Warning: Teacher model does not have a vision tower after initialization.")
        elif model_args.teacher_model_path is not None and model_args.lwf_lambda > 0:
             # This case might occur if the teacher loading failed or the attribute wasn't set correctly
             rank0_print("Warning: LwF is active but teacher model instance not found on student model. Skipping teacher vision init.")
        ...
        if data_args.image_grid_pinpoints is not None:
            if isinstance(data_args.image_grid_pinpoints, str) and "x" in data_args.image_grid_pinpoints:
                ...
                if patch_size not in [224, 336, 384, 448, 512]:
                    # Use globally imported rank0_print; avoid local import which shadows the name
        ...
        # Do that for the teacher model
        if hasattr(model, 'teacher_model') and model.teacher_model is not None:
            model.teacher_model.config.image_aspect_ratio = data_args.image_aspect_ratio
            model.teacher_model.config.image_grid_pinpoints = data_args.image_grid_pinpoints
            model.teacher_model.config.image_crop_resolution = data_args.image_crop_resolution
            model.teacher_model.config.image_split_resolution = data_args.image_split_resolution
            ...
            model.teacher_model.config.mm_newline_position = model_args.mm_newline_position
            model.teacher_model.config.add_faster_video = model_args.add_faster_video
            model.teacher_model.config.faster_token_stride = model_args.faster_token_stride
            model.teacher_model.config.add_time_instruction = data_args.add_time_instruction
            model.teacher_model.config.force_sample = data_args.force_sample
            model.teacher_model.config.mm_spatial_pool_stride = model_args.mm_spatial_pool_stride
        ...
        if model_args.mm_tunable_parts is None:  # traditional way of deciding which part to train
            ...
            if "mm_vision_tower" in tunable_parts:
                if model_args.mm_vision_tower_train_sa_proj_only:
                    vision_sa_proj_params = []
                    for name, param in model.named_parameters():
                        # Skip teacher model parameters entirely
                        if name.startswith("teacher_model."):
                            continue
                        if ("vision_tower" in name) and any(
                            proj_name in name for proj_name in [".q_proj", ".k_proj", ".v_proj", ".out_proj"]
                        ):
                            param.requires_grad_(True)
                            vision_sa_proj_params.append(name)

                    if vision_sa_proj_params:
                        rank0_print("\n==== Trainable Vision Tower Self-Attention Projection Layers ====")
                        layer_projections = {}
                        for param_name in vision_sa_proj_params:
                            if "encoder.layers." in param_name:
                                layer_part = param_name.split("encoder.layers.")[1]
                                layer_num = int(layer_part.split(".")[0])
                                proj_type = param_name.split(".")[-2]
                                if layer_num not in layer_projections:
                                    layer_projections[layer_num] = {}
                                if proj_type not in layer_projections[layer_num]:
                                    layer_projections[layer_num][proj_type] = []
                                layer_projections[layer_num][proj_type].append(param_name)
                        total_params = len(vision_sa_proj_params)
                        rank0_print(f"  Total SA projection parameters: {total_params}")
                        rank0_print(f"  Vision encoder layers with trainable SA projections: {sorted(layer_projections.keys())}")
                        if layer_projections:
                            example_layer = sorted(layer_projections.keys())[0]
                            rank0_print(f"  Example from layer {example_layer} projection types:")
                            for proj_type, params in layer_projections[example_layer].items():
                                rank0_print(f"    - {proj_type}: {len(params)} parameters")
                        rank0_print("==== End of Vision Tower SA Projection Summary ====\n")
                else:
                    for name, param in model.named_parameters():
                        if name.startswith("teacher_model."):
                            continue
            ...
            if "mm_language_model" in tunable_parts:
                ...
                # Support combining both SA projections and MLP gate/up projections
                    ...
                    model_args.mm_language_model_train_mlp_gate_up_only
                    and model_args.mm_language_model_train_attn_proj_only
                ):
                    unfrozen_count = 0
                    mlp_unfrozen, attn_unfrozen, mlp_frozen_down = 0, 0, 0
                    for name, param in model.named_parameters():
                        # Skip teacher, vision and projector parts
                        if name.startswith("teacher_model.") or any(x in name for x in ["vision_tower", "mm_projector", "vision_resampler"]):
                            continue
                        ...
                        if ("model.layers." in name) and (".self_attn." in name) and any(
                            proj_name in name for proj_name in [".q_proj", ".k_proj", ".v_proj", ".o_proj"]
                        ):
                            if not param.requires_grad:
                                rank0_print(f"Unfreezing {name}")
                                param.requires_grad_(True)
                    ...
                    rank0_print(
                        f"Unfrozen {unfrozen_count} parameters in the gate and up projections of MLPs (down projections remain frozen)"
                    )
        ...
        # Save the mm_language_model_train_last_n_layers to the config
        ...
        if model_args.mm_tunable_parts is not None and "mm_language_model" in model_args.mm_tunable_parts.split(",") and model_args.mm_language_model_train_last_n_layers is not None:
            ...
            if transformer_layers:
                expected_layers = set(range(start_layer_idx, num_layers))
        ...
        # We also need to initialize the vision tokenizer for the teacher model
        if hasattr(model, 'teacher_model') and model.teacher_model is not None:
            model.teacher_model.config.mm_use_im_start_end = data_args.mm_use_im_start_end = model_args.mm_use_im_start_end
    ...
    trainer = LLaVALwfTrainer(model=model, tokenizer=tokenizer, args=training_args, **data_module) # Use LLaVALwfTrainer

    # Optional milestone checkpoint saving, aligned with train.py
    class SaveAtStepsCallback(TrainerCallback):
        def __init__(self, steps_list):
            self.steps_set = set(int(s) for s in steps_list if isinstance(s, int) or (isinstance(s, str) and s.isdigit()))
            self._already_saved = set()

        def on_train_begin(self, args, state: TrainerState, control: TrainerControl, **kwargs):
            if len(self.steps_set) > 0:
        ...
        try:
            ...
            if len(milestone_list) > 0:
                ...
                if training_args.max_steps is not None and training_args.max_steps > 0 and training_args.max_steps < largest:
                    rank0_print(
                        f"WARNING: max_steps ({training_args.max_steps}) < largest milestone ({largest}). Some milestone saves may not occur."
                    )
