...
# from llava.model.language_model.llava_qwen_cl_moe_debug import LlavaQwenCLMoEForCausalLM
    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            if "mixtral" in model_name.lower():
                ...
                # import pdb;pdb.set_trace()
                rank0_print(f"In load_pretrained_model, model_name: {model_name}")
                
                # Check for CL MoE model first (takes precedence over regular MoE)
                if "moe_cl" in model_name.lower() or ("model_cls" in kwargs and "LlavaQwenMoECL" in kwargs["model_cls"].__name__):
                    from llava.model.language_model.llava_qwen_moecl_mlp import LlavaQwenMoECLForCausalLM
                    if kwargs["model_cls"].__name__ == "LlavaQwenMoECLForCausalLM":
                        from llava.model.language_model.llava_qwen_moecl_mlp import LlavaQwenMoECLMLPConfig
                        config_cls = LlavaQwenMoECLMLPConfig
                    elif kwargs["model_cls"].__name__ == "LlavaQwenMoECLMLPLoRAForCausalLM":
                        from llava.model.language_model.llava_qwen_moecl_mlp_lora import LlavaQwenMoECLMLPLoRAConfig
                        config_cls = LlavaQwenMoECLMLPLoRAConfig
                    else:
                        raise ValueError(f"Model {model_name} not supported")
                    
                    # Use the provided model_cls if specified
                    model_cls = kwargs.pop("model_cls", LlavaQwenMoECLForCausalLM)
                    rank0_print(f"Loading CL MoE model with class: {model_cls.__name__}")
                    
                    if overwrite_config is not None:
                        llava_cfg = config_cls.from_pretrained(model_path)
                        rank0_print(f"Overwriting config with {overwrite_config}")
                        for k, v in overwrite_config.items():
                            setattr(llava_cfg, k, v)
                        model = model_cls.from_pretrained(model_path, low_cpu_mem_usage=True, attn_implementation=attn_implementation, config=llava_cfg, **kwargs)
                    else:
                        model = model_cls.from_pretrained(model_path, low_cpu_mem_usage=True, attn_implementation=attn_implementation, **kwargs)
                
                # Regular MoE model (non-CL)
