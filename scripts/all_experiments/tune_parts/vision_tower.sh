...
# Environment (ARM-friendly defaults; mirror sa_proj_and_mlp_gate_up.sh)
...
EXPERIMENT_NAME="tune_parts_vision_tower_new_run"
LR=${LR:-5e-6}
VISION_TOWER_LR=${VISION_TOWER_LR:-2e-6}

# Model config (SigLip tower + OV Qwen2 default)
...
for task_index in "${!TASKS[@]}"; do
    ...
    # Ensure vocab_size is present for evaluation
