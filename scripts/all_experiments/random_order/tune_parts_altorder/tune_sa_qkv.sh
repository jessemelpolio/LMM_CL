...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-ov_altorder_tune_sa_qkv}
LR=${LR:-5e-6}
...
echo "Starting sequential training (SA QKV) from base: ${CURRENT_MODEL}"

for task_index in "${!TASKS[@]}"; do
    TASK=${TASKS[$task_index]}
    echo "========== [SA QKV] Training on ${TASK} (${task_index}/${#TASKS[@]}) =========="
    ...
    run_training_stage "${CURRENT_MODEL}" "${TASK}" "${RUN_NAME}" \
        ...
        --mm_language_model_train_attn_qkv_only True
...
echo "[SA QKV] Sequential training complete. Final: ${CURRENT_MODEL}"
