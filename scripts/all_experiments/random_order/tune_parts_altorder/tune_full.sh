...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-ov_altorder_tune_full}
LR=${LR:-5e-6}
...
echo "Starting sequential training (FULL) from base: ${CURRENT_MODEL}"

for task_index in "${!TASKS[@]}"; do
    TASK=${TASKS[$task_index]}
    echo "========== [FULL] Training on ${TASK} (${task_index}/${#TASKS[@]}) =========="
...
echo "[FULL] Sequential training complete. Final: ${CURRENT_MODEL}"
