...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-ov_altorder_tune_sa_proj}
LR=${LR:-5e-6}
...
echo "Starting sequential training (SA PROJ) from base: ${CURRENT_MODEL}"

for task_index in "${!TASKS[@]}"; do
    TASK=${TASKS[$task_index]}
    echo "========== [SA PROJ] Training on ${TASK} (${task_index}/${#TASKS[@]}) =========="
...
echo "[SA PROJ] Sequential training complete. Final: ${CURRENT_MODEL}"
