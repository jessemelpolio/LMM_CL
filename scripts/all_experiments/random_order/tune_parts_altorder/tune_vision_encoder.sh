...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-ov_altorder_tune_vision_encoder}
LR=${LR:-5e-6}
...
echo "Starting sequential training (VISION ENCODER) from base: ${CURRENT_MODEL}"

for task_index in "${!TASKS[@]}"; do
    TASK=${TASKS[$task_index]}
    echo "====== [VISION ENCODER] Training on ${TASK} (${task_index}/${#TASKS[@]}) ======"
...
echo "[VISION ENCODER] Sequential training complete. Final: ${CURRENT_MODEL}"
