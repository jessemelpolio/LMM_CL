...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-ov_altorder_tune_mlp_gate_up}
LR=${LR:-5e-6}
...
echo "Starting sequential training (MLP GATE&UP) from base: ${CURRENT_MODEL}"

for task_index in "${!TASKS[@]}"; do
    TASK=${TASKS[$task_index]}
    echo "====== [MLP GATE&UP] Training on ${TASK} (${task_index}/${#TASKS[@]}) ======"
    ...
    run_training_stage "${CURRENT_MODEL}" "${TASK}" "${RUN_NAME}" \
        ...
        --mm_language_model_train_mlp_gate_up_only True
...
echo "[MLP GATE&UP] Sequential training complete. Final: ${CURRENT_MODEL}"
