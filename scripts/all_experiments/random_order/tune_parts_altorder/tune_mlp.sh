...
EXPERIMENT_NAME=${EXPERIMENT_NAME:-ov_altorder_tune_mlp}
LR=${LR:-5e-6}
...
echo "Starting sequential training (MLP) from base: ${CURRENT_MODEL}"

for task_index in "${!TASKS[@]}"; do
    TASK=${TASKS[$task_index]}
    echo "============= [MLP] Training on ${TASK} (${task_index}/${#TASKS[@]}) ============="
...
echo "[MLP] Sequential training complete. Final: ${CURRENT_MODEL}"
