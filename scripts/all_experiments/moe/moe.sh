...
EXPERIMENT_NAME="moe_baseline_${LLM_VERSION_CLEAN}"
LR=5e-6

if [ "${LLM_VERSION}" == "lmms-lab/llava-onevision-qwen2-0.5b-ov" ]; then
    VOCAB_SIZE=151936
elif [ "${LLM_VERSION}" == "lmms-lab/llava-onevision-qwen2-7b-ov" ]; then
    VOCAB_SIZE=152064
else
    echo "Error: Unsupported model version: ${LLM_VERSION}"
    exit 1
fi
...
add_vocab_size() {
    ...
    python utils/add_vocab_size.py "${checkpoint}" ${VOCAB_SIZE}
}
...
evaluate_all_benchmarks() {
    ...
    -m lmms_eval \
    --model llava_ov_moecl \
    --model_args pretrained=${checkpoint},conv_template=qwen_1_5,model_name=llava_qwen,vocab_size=${VOCAB_SIZE},moe_top_k=1 \
...
for task_index in "${!TASKS[@]}"; do
    ...
    # Train on every stage
    echo "Training on ${TASK}..."
    ACCELERATE_CPU_AFFINITY=1 torchrun --nproc_per_node="${NUM_GPUS}" --nnodes="${NNODES}" --node_rank="${RANK}" --master_addr="${ADDR}" --master_port="${PORT}" \
            llava/train/train_moecl_mlp.py \
            ...
            --route_method "learned_router" \
            --moe_top_k 2 \
            --freeze_original_model True \
            --combined_steps 8 \
            --tuned_model_steps 0 \
            --router_steps 0 \
            --holdout_data_path ${WORKSPACE_DIR}/scripts/all_experiments/blip558k.yaml \
            --moecl_training_stage ${task_index} \
            --original_model_path ${LLM_VERSION}
    ...
    # # Update tie_word_embeddings in config.json to true (required for 0.5B models)
    # echo "Setting tie_word_embeddings to true in config.json for checkpoint: ${LATEST_CHECKPOINT}"
    # python utils/update_tie_embeddings.py "${LATEST_CHECKPOINT}" true
    # if [ $? -ne 0 ]; then
    #     echo "Warning: Failed to update tie_word_embeddings setting"
    # fi
...
# Convert tasks array to space-separated string for command line
TASKS_STR=$(printf "%s " "${TASKS[@]}")
BENCHMARKS_STR="${ALL_BENCHMARKS//,/ }"

# Summary report generation step removed (no local tool provided)
